...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++)
			for (int j = 0; j < image[0].length; j++) {
				int horizontalFlippedIndex = image[0].length - 1 - j;
				flippedImage[i][j] = image[i][horizontalFlippedIndex];
			}
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < image.length; i++)
			for (int j = 0; j < image[0].length; j++) {
				int verticalFlippedIndex = image.length - 1 - i;
				flippedImage[i][j] = image[verticalFlippedIndex][j];
			}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < scaledImage.length; i++) {
			int scaledRowIndex = (int) (i * image.length / height);
			for (int j = 0; j < scaledImage[0].length; j++) {
				int scaledCollumnIndex = (int) (j * image[0].length / width);
				scaledImage[i][j] = image[scaledRowIndex][scaledCollumnIndex];
			}
		}
		return scaledImage;
	}
